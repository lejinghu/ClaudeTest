/*
 * RINGFENCE Defender bot (v0.6 territory rules).
 *
 * It plays the Defender when you play the Attacker, and tools/sim.js uses it
 * for balance numbers. It knows where its own jewels are, as a human
 * Defender does, but only the business flows Security Intelligence has
 * shown it.
 *
 * Strategies:
...
(function (root, factory) {
  const R = root.RF || (typeof require === 'function' ? require('./rules.js') : null);
  const A = root.RFAI || (typeof require === 'function' ? require('./ai.js') : null);
  const Bot = factory(R, A);
  if (typeof module === 'object' && module.exports) module.exports = Bot;
  else root.RFBOT = Bot;
})(typeof self !== 'undefined' ? self : this, function (RF, AI) {
  'use strict';
  ...
  return { chooseAction, mindless, danger, STRATEGIES: ['careful', 'territory', 'fortress', 'infra', 'hasty', 'mindless'] };
});
